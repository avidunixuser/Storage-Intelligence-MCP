...
@description('The name of the Flex Functions subnet')
param functionsSubnetName string = 'functions-subnet'
...
@description('Address prefix for the Flex Functions subnet (only needed if creating new subnet)')
...
@description('Set to true to reference the Flex Functions subnet as existing')
param functionsSubnetExists bool = false
...
var functionsSubnetSpaces = empty(functionsSubnetPrefix) ? cidrSubnet(vnetAddressSpace, 24, 3) : functionsSubnetPrefix
...
module functionsSubnet 'subnet.bicep' = {
  name: 'functions-subnet-${uniqueString(deployment().name, functionsSubnetName)}'
  scope: resourceGroup(vnetResourceGroupName)
  params: {
    vnetName: vnetName
    subnetName: functionsSubnetName
    addressPrefix: functionsSubnetSpaces
    subnetExists: functionsSubnetExists
    delegations: [
      {
        name: 'Microsoft.App/environments'
        properties: {
...
output functionsSubnetId string = functionsSubnet.outputs.subnetId
